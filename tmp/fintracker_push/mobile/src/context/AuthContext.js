...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (token) {
          // Check MFA setting - require biometric if enabled
          const mfaEnabled = await AsyncStorage.getItem('mfa_enabled');
          if (mfaEnabled === 'true') {
  ...
  async function logout() {
    ...
    await removeToken();
    // Clear custom pages on logout
    try { await AsyncStorage.removeItem('custom_pages'); } catch (_) {}
    setUser(null);
  }
