...
// Key data fields per collection for entry forms
const ENTRY_FIELDS = {
  banks: ['date', 'name', 'type', 'description', 'expenseCategory', 'amount'],
  cardusage: ['date', 'name', 'type', 'description', 'expenseCategory', 'amount'],
  carloan: ['name', 'bank', 'amount', 'monthlyPayment'],
  category: ['name', 'type'],
  commitment: ['name', 'amount', 'frequency', 'category'],
  epf: ['date', 'employee', 'employer', 'totalBalance'],
  expensesummary: ['month', 'totalIncome', 'totalExpense', 'savings'],
  fd: ['bank', 'amount', 'interestRate', 'maturityDate'],
  houseloan: ['name', 'bank', 'amount', 'monthlyPayment'],
  houseloaninfo: ['name', 'purchasePrice', 'currentValue', 'loanAmount'],
  insurance: ['name', 'provider', 'type', 'premium'],
  insuranceinvestment: ['name', 'provider', 'premium', 'currentValue'],
  investment: ['name', 'type', 'amount', 'currentValue'],
  share: ['name', 'code', 'units', 'buyPrice'],
  sspn: ['date', 'amount', 'totalBalance', 'type'],
};
...
export default function TemplatesScreen() {
      ...
      const payload = {
        ...
        entries: validEntries.map((e) => {
          ...
          Object.entries(e.data).forEach(([k, v]) => {
            if (v && v.trim()) {
              const numFields = ['amount', 'monthlyPayment', 'interestRate', 'premium',
                'coverage', 'currentValue', 'totalBalance', 'employee', 'employer',
                'totalIncome', 'totalExpense', 'savings', 'purchasePrice', 'loanAmount',
                'units', 'buyPrice', 'currentPrice'];
  ...
  function getEntryDisplayText(entry) {
    ...
    const parts = [];
    const keys = ['name', 'description', 'amount', 'date', 'type'];
    keys.forEach((k) => {
    ...
    if (parts.length === 0) {
      keys.forEach((k) => {
        if (entry[k]) parts.push(`${k}: ${entry[k]}`);
      });
    }
                    ...
                    getEntryFields(entry.collection).map((fieldKey) => (
                      <View key={fieldKey}>
                        ...
                        <TextInput
                          ...
                          placeholderTextColor={COLORS.textSecondary}
                          keyboardType={
                            ['amount', 'monthlyPayment', 'interestRate', 'premium',
                              'coverage', 'currentValue', 'totalBalance', 'employee',
                              'employer', 'totalIncome', 'totalExpense', 'savings',
                              'purchasePrice', 'loanAmount', 'units', 'buyPrice',
                              'currentPrice'].includes(fieldKey)
                              ? 'decimal-pad'
                              : 'default'
                          }
